...
export function AdminPanelLayout({
  ...
  // Convert consolidated config to menu items with icon components
  const menuItems: MenuItem[] = adminMenuItems.map(item => ({
    id: item.id,
    label: item.label,
    icon: iconMap[item.icon] || BarChart3, // Fallback to BarChart3 if icon not found
    href: item.href,
    section: item.section,
    description: item.description,
  }));

  // Convert sections from consolidated config
  const sections = Object.entries(ADMIN_MENU_SECTIONS).map(([key, title]) => ({
    title,
    key,
  }));
